...
if($this->config->get('mta_ypx_status')) {	
	if(isset($data['affiliate_id']) && mta_check_int($data['affiliate_id']) && $data['affiliate_id'] > 0) $this->db->query("REPLACE INTO " . DB_PREFIX . "mta_customer (customer_id, parent_affiliate_id) VALUES ('" . (int) $customer_id . "', '" . (int) $data['affiliate_id'] . "')");

	if(isset($data['affiliate']) && !isset($data['payment'])) $data['payment'] = '';
}
//+mod by yp end
...
if($this->config->get('mta_ypx_status')) $sql = "SELECT c.*, cgd.*, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group, mta_c.parent_affiliate_id AS affiliate_id FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "mta_customer mta_c ON mta_c.customer_id = c.customer_id "; //+mod by yp
...
if($this->config->get('mta_ypx_status')) {			
	...
	return $this->model_extension_mta_affiliate->getAffiliatesMta($data);
}	
//+mod by yp end
...
if($this->config->get('mta_ypx_status')) {				
	$this->load->model('extension/mta/affiliate');						
	return $this->model_extension_mta_affiliate->getTotalAffiliatesMta($data);
}	
//+mod by yp end
