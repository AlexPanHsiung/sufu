...
class ControllerCatalogProduct extends Controller {
	...
	private function _check_coupons($max_comm, $price, $coupon_id) {
		...
		if(isset($coupons[$coupon_id])) {
			$discount = floatval((strtolower($coupons[$coupon_id]['type']) === 'f' ? $coupons[$coupon_id]['discount'] : ($price * 0.01 * $coupons[$coupon_id]['discount'])));
			if($max_comm >= ($price - $discount)) return false;
		}
		return true;		
	}
	...
	private function _count_product_scheme_usage($product_id) {		
		return array_merge($this->model_extension_mta_mta_product_affiliate->countProductUsage($product_id, 'product_affiliate'), $this->model_extension_mta_mta_product_affiliate->countProductUsage($product_id));
	}
	...
	public function get_affiliates() {
		...
		if($product_id && mta_check_int($product_id)) {
			$mod_type = isset($this->request->request['mod_type']) && in_array($this->request->request['mod_type'], array('coupon', 'discount', 'special')) ? $this->request->request['mod_type'] : '';
  			...
  			$ret = $this->model_extension_mta_mta_product_affiliate->getProductAffiliates($product_id, $mod_type, $mod_id);
		} else {
			$ret = '0';
		}
	...
	if($this->config->get('mta_ypx_status')) {	
      ...
      $category =  $this->model_catalog_product->getProductCategories($result['product_id']);
		...
		if($this->config->get('mta_ypx_status')) {
			$data['mta_status'] = true;
			$_s = $this->model_extension_mta_mta_product_affiliate->getProductScheme($result['product_id']);
...
//+mod by yp start
	if($this->config->get('mta_ypx_status')) {
		...
		$this->load->model('extension/mta/mta_product');		
		$data['schemes'] = $this->_get_schemes();	
		$data['schemes_jsstr'] = array();
		$data['schemes0'] = array('0' => $this->language->get('text_default'));
		$data['schemes0_jsstr'] = array('0' => mta_jsstr($this->language->get('text_default')));
		
		foreach($data['schemes'] as $_k => $_v) {
			$__v = mta_jsstr($_v);
			$data['schemes_jsstr'][$_k] = $__v;
			$data['schemes0'][strval($_k)] = $_v;
			$data['schemes0_jsstr'][strval($_k)] = $__v;
		}		
		$data['tab_scheme_jsstr'] = mta_jsstr($this->language->get('tab_scheme'));
	...
	if($this->config->get('mta_ypx_status')) {
		...
		if($_do_edit) $data['link_commission_tab'] = mta_jsstr($this->url->link('extension/module/mta/mta_pds/product_commission', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $data['product_id'], true));
	} else {
		$data['mta_status'] = false;
	}
	//+mod by yp end
	...
	protected function validateForm() {
		...
		if($this->config->get('mta_ypx_status')) {
			...
			if(isset($this->request->request['product_id'])) {
				$_schemes = $this->model_extension_mta_mta_product->getSchemes(array('fields' => array('mta_scheme_id as id', 'commission_type')));
				$schemes = array();			
				foreach($_schemes as $_s) {				
					if($_s['commission_type'] == 'fixed') $schemes[strval($_s['id'])] = $this->model_extension_mta_mta_product->getMaxTotalCommission($_s['id']);
				} 
				if(sizeof($schemes) > 0) {					
					$p =& $this->request->post;
