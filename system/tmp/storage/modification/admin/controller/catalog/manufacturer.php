...
class ControllerCatalogManufacturer extends Controller {
	...
	if($this->config->get('mta_ypx_status')) {
		...
		if(isset($this->request->get[$data['page_type'] . '_id']) && $this->request->get[$data['page_type'] . '_id']) {
			...
			$data['mta_pds_url'] = mta_jsstr($this->url->link('extension/module/mta/mta_pds', 'user_token=' . $this->session->data['user_token'] . '&type=' . $etype . '&id=' . intval($this->request->get[$data['page_type'] . '_id']), true));
		}
	} else {
