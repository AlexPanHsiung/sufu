	...
	public function index($route, $args, $output) {
		...
		if($this->config->get('mta_ypx_status')) {
			...
			if($this->config->get('module_aff_classic_status') && $this->config->get('module_aff_classic_commission_table')) {
				...
				$_tt = $this->model_extension_aff_classic->getTransactionTotal($customer_id);
			} else {
				$_tt = $this->model_customer_customer->getTransactionTotal($customer_id);
			}
			$message .= sprintf($this->language->get('text_balance'), $this->currency->format($_tt, $this->config->get('config_currency')) );
		} else {
