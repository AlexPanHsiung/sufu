...
class ControllerSaleOrder extends Controller {
	...
	protected function _addMtaCommission($order_id, $affiliate_id) {
		...
		if($this->model_extension_mta_affiliate->addMtaTransaction($order_id, $affiliate_id, $this->language->get('text_order_id') . ' #' . $order_id)) {
	...
	protected function _removeMtaCommission($order_id, $affiliate_id) {
		...
		$this->load->model('extension/mta/affiliate');
		if($this->model_extension_mta_affiliate->deleteMtaTransaction($order_id, $affiliate_id)) {
			$json['success'] = $this->language->get('text_commission_added');
		} else {
			$json['error'] = $this->language->get('error_action');
		}		
	...
	public function getForm() {
		...
		if($this->config->get('mta_ypx_status')) {
			...
			if(isset($this->request->request['order_id'])) {
				$data['link_edit_order_commission'] = $this->url->link('extension/module/mta/mta/order_commission', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->request['order_id'], true);
			} else {
				$data['link_edit_order_commission'] = '';
			}
			$data['text_edit_order_commission_jsstr'] = mta_jsstr($this->language->get('text_edit_order_commission'));			
			$data['link_edit_order_commission_jsstr'] = mta_jsstr($data['link_edit_order_commission']);			
		} else {
	...
	public function info() {
		...
		//+mod by yp end
			...
			$data['text_this_month_cost'] = $this->language->get('text_this_month_cost');
			$data['text_notice_transaction'] = $this->language->get('text_notice_transaction');
			$data['this_month_cost'] = $this->model_sale_order->getThisMonthTotal($order_info['customer_id']);
...
if($this->config->get('mta_ypx_status')) {
	...
	$data['link_edit_order_commission'] = $this->url->link('extension/module/mta/mta/order_commission', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->request['order_id'], true);
	$data['text_edit_order_commission_jsstr'] = mta_jsstr($data['text_edit_order_commission']);	
	$data['link_edit_order_commission_jsstr'] = mta_jsstr($data['link_edit_order_commission']);
} else {
	$data['mta_status'] = false;
}
...
//+mod by yp end
		...
		if($data['mta_status']) {
			...
			$this->load->model('extension/mta/affiliate');
			$_aff_infos = $this->model_extension_mta_affiliate->getOrderCommissions($this->request->get['order_id']);
			if ($order_info['affiliate_id'] || $_aff_infos) {
				if(!$_aff_infos) {
					...
					if($this->config->get('module_aff_classic_status') && $this->config->get('module_aff_classic_commission_table')) {
						...
						$data['commission_total'] = $this->model_extension_aff_classic->getTotalTransactionsByOrderId( $this->request->get['order_id'] ); 			
					} else {
						$data['commission_total'] = $this->model_customer_customer->getTotalTransactionsByOrderId( $this->request->get['order_id'] ); 
					}
				} else {
					...
					foreach($_aff_infos as $_aff_info) {
						$__ar = array(
							...
							'affiliate_link' => $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $_aff_info['affiliate_id'], true),
							'affiliate_scheme_id' => $_aff_info['scheme_id'],
							'affiliate_scheme_name' => ($_aff_info['scheme_id'] ? $_aff_info['scheme_name'] : ''),
							'affiliate_scheme_link' => ($_aff_info['scheme_id'] ? $this->url->link('extension/module/mta/mta/update', 'user_token=' . $this->session->data['user_token'] . '&id=' . $_aff_info['scheme_id'], true) : ''),
		...
		//+mod by yp end
			...
			//toastguo custom
			if($data['affiliates']){
				foreach($data['affiliates'] as $index => $aff_data){
					$data['affiliates'][$index]['this_month_cost'] = $this->model_sale_order->getThisMonthTotal($aff_data['affiliate_id']);
				}
	...
	public function addCommission() {
		...
		//+mod by yp start
		if($this->config->get('mta_ypx_status') && isset($this->request->request['affiliate_id'])) {
