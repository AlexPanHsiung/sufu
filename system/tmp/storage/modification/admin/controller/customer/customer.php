...
//+mod by yp start
if($this->config->get('mta_ypx_status') && sizeof($data['customers'])) {
	$data['mta_status'] = true;
	$this->load->model('extension/mta/affiliate');
	$_aff_ids = array();
	foreach($data['customers'] as $_c) {
		$_aff_ids[] = (int)$_c['customer_id'];
	}
	$_levels = $this->model_extension_mta_affiliate->getLevels($_aff_ids);
	foreach($data['customers'] as $_i => $_c) {
		$data['customers'][$_i]['level'] = isset($_levels['a' . $_c['customer_id']]) ? $_levels['a' . $_c['customer_id']] : '';
	}
} else {
	$data['mta_status'] = false;
}
// +mod by yp end
	...
	if($this->config->get('mta_ypx_status')) {
		if (isset($this->request->get['customer_id']) && 	($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$affiliate_info_mta = $this->model_extension_mta_mta_affiliate->getAffiliate($this->request->get['customer_id'], true);	
		}
		...
		$data['schemes_jsstr'] = $data['schemes'];
		foreach($data['schemes_jsstr'] as $_i => $_name) {
			$data['schemes_jsstr'][$_i] = mta_jsstr($_name);
		}
    	$this->load->model('extension/mta/mta_affiliate');		
		foreach(array('text_level', 'text_view_downline', 'entry_parent', 'entry_affiliate', 'text_none', 'text_select_affiliate', 'text_set_none', 'entry_scheme', 'option_default') as $_v) {
			$data[$_v . '_jsstr'] = mta_jsstr($this->language->get($_v));
		}
	} else {
		$data['mta_status'] = false;
	}
	...
	//+mod by yp start
	if($this->config->get('mta_ypx_status')) {
		//$data['entry_affiliate_jsstr'] = mta_jsstr($this->language->get('entry_affiliate');
		$data['mta_status'] = true;
		...
		if($data['affiliate_id'])	{
			$this->load->model('customer/customer');
			$_aff = $this->model_customer_customer->getAffiliate($data['affiliate_id']);
			if($_aff) $_aff = $this->model_customer_customer->getCustomer($data['affiliate_id']);
		...
		if($data['affiliate_id']) {
			$data['affiliate_link'] = $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $data['affiliate_id'], true);			
			$data['affiliate_link_jsstr'] = mta_jsstr($data['affiliate_link']);
			$data['affiliate_name_jsstr'] = mta_jsstr($data['affiliate_name']);
		}
	} else {
		$data['mta_status'] = false;
	}
...
if($this->config->get('mta_ypx_status')) {
	...
	if(isset($affiliate_info_mta['level_original'])) {
		...
		$data['link_downline'] = $this->url->link('extension/module/mta/affiliate_downline', 'user_token=' . $this->session->data['user_token'] . '&affiliate_id=' . $affiliate_info_mta['id'], true);
		$data['link_downline_jsstr'] = mta_jsstr($data['link_downline']);
	} else {
		$data['text_view_downline'] = '';
		$data['link_downline'] = $data['link_downline_jsstr'] = '';
	}	
    ...
    if($data['parent_affiliate_id']) {
		$data['parent_affiliate_link'] = $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $data['parent_affiliate_id'], true);
		$data['parent_affiliate_link_jsstr'] = mta_jsstr($data['parent_affiliate_link']);
	}
	...
	$data['parent_affiliate_name_jsstr'] = mta_jsstr($data['parent_affiliate_name']);
