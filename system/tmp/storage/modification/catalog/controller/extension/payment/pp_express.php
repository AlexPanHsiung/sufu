...
if($this->config->get('mta_ypx_status')) {		
	$data['mta_status'] = true;			
			if (isset($this->request->post['txn_type']) && ($this->request->post['txn_type'] == 'recurring_payment' || $this->request->post['txn_type'] == 'recurring_payment_outstanding_payment') && $this->config->get('mta_ypx_pay_recur')) {
				$profile = $this->model_account_recurring->getOrderRecurringByReference($this->request->post['recurring_payment_id']);			
				if ($profile) {					
					$_res = $this->db->query("SELECT mta_op.affiliate_id, mta_op.commission, op.total FROM " . DB_PREFIX . "mta_order mta_o, " . DB_PREFIX . "mta_order_product mta_op, " . DB_PREFIX . "order_product op WHERE mta_op.product_id = '" . (int)$profile['product_id'] . "' AND mta_o.order_id = '" . (int)$profile['order_id'] . "' AND mta_op.mta_order_id = mta_o.mta_order_id AND op.order_product_id = mta_op.order_product_id");
					$_comms = array();
					...
					if(sizeof($_comms) > 0) {
						...
						foreach($_comms as $_aff_id => $_comm) {
							$this->model_account_customer->addTransaction($_aff_id, sprintf($this->language->get('text_recurring_commission'), strval($profile['order_id'])), $_comm, $profile['order_id']);
						}
					}				
				}
			}
