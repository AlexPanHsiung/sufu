...
class Productalert {
      ...
      if (strpos($_SERVER['PHP_SELF'], '\/admin\/') !== false) {
        ...
        $link = $this->url->link('product/product&product_id=' .$product_id, '',true);
  		...
  		if ($notify == 1) { // this one to notify customer accept massage
  			...
  			$customer = $this->db->query("SELECT * FROM " . DB_PREFIX . "pricealert_quote WHERE product_id = '" . $product_id . "' AND quote_id = '" . (int)$request_id . "' AND customer_email = '" . $this->db->escape($email) . "'")->row;
  			...
  			$this->db->query("UPDATE " . DB_PREFIX . "pricealert_quote SET status = '1', responded = '1', accept = '1', coupon_id = '" . $coupon_id . "' WHERE product_id = '" . (int)$product_id . "' AND  quote_id = '" . (int)$request_id . "' AND customer_email = '" . $this->db->escape($email) . "'");
  			...
  			$this->db->query("UPDATE " . DB_PREFIX . "pricealert_quote SET status = '1', responded = '1', reject = '1' WHERE product_id = '" . $product_id . "' AND  quote_id = '" . $request_id . "' AND customer_email = '" . $email . "'");

  			$details = $this->productDetails($product_id);

  			$customer = $this->db->query("SELECT customer_name FROM " . DB_PREFIX . "pricealert_quote WHERE product_id = '" . $product_id . "' AND  quote_id = '" . $request_id . "' AND customer_email = '" . $email . "'")->row;
        if (strpos($_SERVER['PHP_SELF'], '\/admin\/') !== false) {
          $link = HTTP_CATALOG.'index.php?route=product/product&product_id='. $product_id;
        } else {
          $link = $this->url->link('product/product&product_id=' .$product_id, '',true);
  ...
  */
 	public function insertQuote($data) {
 		...
 		$exist = $this->db->query("SELECT quote_id, product_option, requests FROM " . DB_PREFIX . "pricealert_quote WHERE customer_email = '" . $customer_email . "' AND product_id = '" . $data['product_id'] . "'")->row;
    $vendor_id = 0;
    if($this->config->get('module_marketplace_status')){
      $vendor_id = $this->getSellerByProduct($data['product_id']);
    }
 		if (isset($exist['quote_id']) && $exist['product_option'] == $data['option']) {
  ...
  public function priceAlertSelectedOption($product_id) {
    $sql = "SELECT product_selected_option FROM " . DB_PREFIX . "pricealert_products WHERE product_id = '" . $product_id . "' and status = 1 and date_till >= NOW()";
