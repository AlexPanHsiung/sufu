...
class ControllerCustomerpartnerSell extends Controller {
	...
	public function index() {
	  ...
	  $this->data['soconfig'] = $this->soconfig;
		$this->data['theme_directory'] = $this->config->get('theme_default_directory');
		if(isset($_COOKIE["listingType"]) && $_COOKIE["listingType"]){$this->data['listingType'] = $_COOKIE["listingType"]; }
	  else{$this->data['listingType'] = $this->data['soconfig']->get_settings('product_catalog_mode') ? 'list' : 'grid';}
		...
		foreach($latest as $key => $result){
          ...
          ,'quick_view'  => $this->config->get('config_ssl').'index.php?route=extension/soconfig/quickview&product_id='.$result['product_id']
