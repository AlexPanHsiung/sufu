(function($) {
    ...
    let $deactivateForm  = jQuery('#faq-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('faq-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'faq-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('faq-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_faq.php', jQuery.param(data)).then(function(result) {
