  ...
  public function __addPermissions() {
	...
	public function index() {
		...
		$this->model_customerpartner_partner->upgradeMarketplace();
		 ...
		 $event_helper = $this->model_setting_event;
		 if(empty($event_helper->getEventByCode('topserch_marketplace'))) {
		  	$event_helper->addEvent('topserch_marketplace',
			'catalog/model/catalog/product/getProducts/after', 'account/customerpartner/topsearch/_processSearch');
		 }
		
		 if(empty($event_helper->getEventByCode('limit_product_order'))) {
		  	$event_helper->addEvent('limit_product_order',
 		 'catalog/controller/checkout/cart/add/before', 'account/customerpartner/event_handler/handleCartRequest');
		 }
		 
		 if(empty($event_helper->getEventByCode('limit_update_product_cart'))) {
			$event_helper->addEvent('limit_update_product_cart',
 		 'catalog/controller/checkout/cart/edit/before', 'account/customerpartner/event_handler/handleCartUpdate');
	   }
		 ...
		 if(empty($event_helper->getEventByCode('mpCommonMenuLink'))) {
			$event_helper->addEvent($code, $trigger, $action);
	   }
		 ...
		 if(empty($event_helper->getEventByCode($code))) {
			$event_helper->addEvent($code, $trigger, $action);
	   }
