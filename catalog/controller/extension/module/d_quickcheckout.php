...
$this->load->model('extension/d_quickcheckout/address');
    ...
    public function createOrder(){
        ...
        if($affiliate_info && !$affiliate_info['status']){
            ...
            $thisMonth = $this->model_checkout_order->getThisMonthTotal($this->customer->getId());
            $thisMonth += $total;
            //echo '**** this month total ' . print_r($total, 1) . '<br/>';

            if($thisMonth >= 1000){
                $this->model_account_customer->setAffiliateStatus($this->customer->getId(), true);
            }else{
