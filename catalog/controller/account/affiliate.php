...
class ControllerAccountAffiliate extends Controller {
	...
	private function add_customer_group_id() {
		$this->load->model('account/customer');
		$this->model_account_customer->editCustomerGroupId($this->customer->getId(), 3);
	}
	...
	public function getForm() {
		...
		if (isset($this->error['bank_branch_number'])) {
			$data['error_bank_branch_number'] = $this->error['bank_branch_number'];
		} else {
			$data['error_bank_branch_number'] = '';
		}
	...
	protected function validate() {
		if ($this->request->post['payment'] == 'cheque' && !$this->request->post['cheque']) {
			...
			if($this->request->post['bank_branch_number'] == '') {
				$this->error['bank_branch_number'] = $this->language->get('error_bank_branch_number');
			}
