  ...
  public function uploadAchivments() {
   ...
   $filename = $this->pdfValidate(1);
	...
	private function pdfValidate($type = ''){
		...
		if(isset($_FILES["seller_achivments"])){
			...
			$target_file = $directory . basename($_FILES["seller_achivments"]["name"]);
			//check for the multiple extension
			$multiple = explode('.',$_FILES["seller_achivments"]["name"]);
      if (count($multiple) == 2) {

				if (!empty($_FILES['seller_achivments']['name']) && is_file($_FILES['seller_achivments']['tmp_name'])) {
					// Sanitize the filename
					$filename = basename(html_entity_decode($_FILES['seller_achivments']['name'], ENT_QUOTES, 'UTF-8'));
					...
					$content = file_get_contents($_FILES['seller_achivments']['tmp_name']);
					...
					if ($_FILES['seller_achivments']['error'] != UPLOAD_ERR_OK) {
						$json['error'] = true;
					}
				}
			} 
    ...
    if($type) {
			if(!$json['error']) {
				move_uploaded_file($_FILES['seller_achivments']['tmp_name'], $directory . '/' . $filename);
