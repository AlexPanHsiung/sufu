...
class ModelCustomerpartnerMaster extends Model {
	...
	public function getAverageFeedback($customerid, $field_id = 0){
	  ...
	  $sql = "SELECT (AVG(field_value)) avg  FROM `" . DB_PREFIX . "wk_feedback_attribute_values` WHERE feedback_id IN (SELECT id FROM `".DB_PREFIX."customerpartner_to_feedback` WHERE seller_id='".(int)$customerid."' AND status = '1')";
